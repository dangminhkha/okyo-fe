        ...
        <v-carousel v-if="imageList.length > 0" hide-delimiters hide-delimiter-background height="300" :cycle="true"
          ...
          <v-carousel-item cover v-for="(item, index) in imageList" :key="index">
            <img :src="API_URL + item.path"
              class="m-auto max-h-[400px] h-[100%] hover:-translate-y-1 hover:scale-105 duration-300" /></v-carousel-item>
        </v-carousel>
        <div v-else>
      ...
      <div class="p-3 border rounded-lg">
        ...
        <div v-if="dataDetail.details">
          ...
          <ul class="list-disc list-inside">
            <li v-for="(item, index) in dataDetail.details" :key="index" class="text-gray-500">
              {{ item }}
            </li>
          </ul>
        </div>
        ...
        <v-row>
          <v-col cols="12" md="6">
            <v-select label="Đại lý BH" :items="agentData" v-model="agentSelected" variant="outlined"
              density="comfortable" item-title="name" item-value="id" return-object clearable></v-select>
          </v-col>
          <v-col cols="12" md="6">
  ...
  data() {
    return {
      ...
      rules: {
        ...
        varChar: (v) => !v || /^[a-zA-Z0-9]*$/i.test(v) || 'Không nhập ký tự đặc biệt',
      },
      dialogDetail: false,
      dialogDetailId: null,
      dialogRemove: false,
