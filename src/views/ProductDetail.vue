<template>
  ...
  <v-dialog v-model="addAgentDialog" max-width="500" persistent>
    <div class="bg-white p-4 rounded-lg">
      <div class="text-right" @click="addAgentDialog = false">
        <span class="mdi mdi-close cursor-pointer font-bold text-2xl"></span>
      </div>

      <div class="text-center text-xl font-bold mb-4">
        Cập nhật đại lý cho sản phẩm
      </div>
      <v-form
        ref="formAgent"
        v-model="validAgent"
      ...
      <div class="flex justify-center my-4">
        <v-btn variant="flat" color="blue-darken-4" @click="submitAddAgent">
          Xác nhận
        </v-btn>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    addAgentToProduct(data) {
      this.addAgentDataSelected = data;
      this.agentSelected = data.salesAgent;
      this.addAgentDialog = true;
    },
    async submitAddAgent() {
      const { valid } = await this.$refs.formAgent.validate();
      if (valid) {
        let params = {
          id: this.addAgentDataSelected.id,
          productId: this.dataDetail.id,
          code: this.addAgentDataSelected.code,
          salesAgentId: this.agentSelected ? this.agentSelected.id : "",
        };
