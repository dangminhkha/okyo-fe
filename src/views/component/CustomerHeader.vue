<template>
  ...
  >
    <nav
      class="px-4 sm:px-6 flex justify-between basis-full items-center w-full mx-auto"
    >
      <div>
        <!-- Logo -->
        <router-link
      ...
      >
        ...
        <v-menu>
          ...
          <v-list>
            <v-list-item
              v-for="(item, i) in getListTypeData.data"
              :key="i"
              @click="menuSelect(item.id)"
            >
              <v-list-item-title
                class="text-blue-darken-3 font-bold cursor-pointer"
